...
class DatabaseHelper {
  ...
  Future _onCreate(Database db, int version) async {
    ...
    await db.execute(
        '''CREATE TABLE $tableSyncOneWay4 (id INTEGER,TenentID INTEGER,LOCATION_ID INTEGER,planid INTEGER,MealType INTEGER,UOM INTEGER,
    ...
    String refTableQueryStr =
        '''CREATE TABLE $tableRefTable( 
    TenentID INTEGER 
    ,REFID INTEGER
    ,REFTYPE TEXT
    ,REFSUBTYPE TEXT 
  ...
  Future getMealName(int mealType) async {
    ...
    return await db.rawQuery(
        'SELECT REFNAME1 FROM $tableRefTable WHERE REFID = $mealType');
  }
  ...
  Future<void> insertToSyncOneWay4(List syncOneWay4) async {
    ...
    syncOneWay4.forEach((element) async {
      await db.transaction((txn) => txn.insert(
          tableSyncOneWay4, element.toMap(),
  ...
  Future queryPlanMealWithPlanId(String planId) async {
    ...
    var rows = await db.rawQuery(
        'SELECT DISTINCT plandays FROM $tableSyncOneWay4 WHERE planid = $planId');
    return rows;
  }
  ...
  Future<List<Map<String, dynamic>>> queryAllRowsFromPlanMealCustInvoice(
      int planId, int rMyTransID) async {
    Database db = await instance.database;
    var query =
        'SELECT ${TableFields.expectedDelDate}, ${TableFields.nameOfDay}, ${TableFields.totalWeek}, ${TableFields.expectedDeliveryDay} FROM $tablePlanMealCustInvoice WHERE ${TableFields.mytransid}=$rMyTransID AND ${TableFields.mealType}=1402';
    print(query);
  ...
  selectPlanMealWithPlaId(var planId) async {
    Database db = await instance.database;
    return await db
        .rawQuery('SELECT* FROM $tablePlanMeal WHERE planid = $planId');
  }

  selectPlanMealWithGroupId(var groupId) async {
    Database db = await instance.database;
    return await db
        .rawQuery('SELECT* FROM $tablePlanMeal WHERE GroupId = $groupId');
  }
  ...
  Future<List<Map<String, dynamic>>> querySelectedPlanMealCustInvoice(
    ...
    Database db = await instance.database;
    return await db.rawQuery(
        '''SELECT * FROM $tablePlanMealCustInvoice WHERE 
  ...
  Future getAllDistinctTransId() async {
    Database db = await instance.database;
    return await db.rawQuery(
        'SELECT DISTINCT ${TableFields.transId} FROM $tablePlanMealCustInvoice ');
  }
  ...
  deleteFromMoreHD(int id) async {
    Database db = await instance.database;
    return await db.rawQuery(
        "DELETE FROM $tablePlanMealCustInvoiceMoreHD WHERE $columnId=$id");
  }
